...
public interface IResearchSeedbedProfileRepository extends JpaRepository<ResearchSeedbedProfileEntity, Long> {
    ...
    @Query(value = """
        SELECT 
            ap.name AS periodoAcademico,
            ig.name AS grupoDeInvestigacion,
            rs.name AS semillero,
            u_coordinator.full_name AS coordinador,
            u_student.full_name AS nombreDelEstudiante,
            u_student.user_code AS codigo,
            apg.name AS nombreDelProgramaAcademico,
            sp.semester AS semestre,
            u_student.sex AS sexo
